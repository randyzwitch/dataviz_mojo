...
case_hashes = {case: hashlib.sha256((prototype/'benchmarks/issue607'/(case+'.mojo')).read_bytes()).hexdigest() for case in cases}
...
for run in range(3):
    for case in cases:
        for name in (['baseline','prototype'] if run % 2 == 0 else ['prototype','baseline']):
            tree=trees[name]
            assert hashlib.sha256((prototype/'benchmarks/issue607'/(case+'.mojo')).read_bytes()).hexdigest() == case_hashes[case], 'Benchmark source changed'
