...
(see the wiki). LINE's `Theme.line_smoothing` (default
0.0, plain straight segments) optionally curves it through a Catmull-
Rom-derived spline instead -- see that field's docstring and
...
marks alongside BAR: LOLLIPOP reuses BAR's `encode_categorical`
data shape unchanged (a stem + point instead of a filled rect is
purely a rendering difference); WATERFALL has its `encode_
waterfall` (a category + a *signed delta*, not a plain value -- see
that method's docstring for the running-total bookkeeping this
...
narrower rising/falling delta bar); BOX has its `encode_boxplot`
(a category + a whole
*distribution* of raw values, not one number -- see that method's docstring for the quartile/whisker/outlier computation it does up
...
function's docstring for why sharing became the right call once a
third and fourth mark type needed the identical layout.

CANDLESTICK and BULLET ("Phase 2b", the marks that need their new
shape rather than building on BAR's data directly -- see
the wiki) are a fifth and sixth categorical-x-axis mark
sharing that same axis-frame core. CANDLESTICK's `encode_
...
`_render_candlestick`'s docstring for the drawing order and why it
reuses `Theme.mark_color`/`mark_color_negative` unconditionally rather
than adding dedicated bullish/bearish fields. BULLET's `encode_
...
`_render_bullet`'s docstring for the drawing order and why, unlike
...
type of its own -- see that method's docstring). Shares nothing
...
one now `y`) -- see its mirror-image core, `_draw_horizontal_
...
(series, category) pair) -- back on `_draw_categorical_axis_frame`'s shared vertical axis-frame core (categories still run one way, the
same as BAR/LOLLIPOP/etc.), just subdividing each category's band
...
bar`'s docstring for the palette/legend-reservation details.
...
same call LOLLIPOP's reuse of BAR's `encode_categorical` already
established the precedent for -- identical data, purely a rendering
difference) -- each category's series stack vertically instead of
sitting side by side: full band width per segment, one segment on top
of the previous one's running total instead of `GROUPED_BAR`'s
divided sub-bars. See `_render_stacked_bar`'s docstring for the
mixed-sign running-total bookkeeping (positive and negative values
stack in their direction from zero, independently) and why no
extra pixel-boundary-rounding trick is needed here the way `GROUPED_
BAR`'s sub-bar division needed one.

POPULATION_PYRAMID ("Phase 3" of the broader chart-type survey, the
first of the ECharts-parity gap-closing marks) is `GANTT`'s horizontal categorical axis frame (`_draw_horizontal_categorical_axis_
...
`GANTT`'s span does -- see that method's docstring, and `_render_
...
axis's domain via `_categorical_indices`, the same helper `Mark.
POINT`'s categorical color channel already uses) draws a filled
cell per row, colored through a continuous `ColorScale` spanning
`value`'s [min, max] -- `Theme.color_scale_low`/`color_scale_high`,
the exact gradient `Mark.POINT`'s continuous `color=` channel
...
function's docstring for why, and for the `padding=0.0` choice
that makes cells tile edge-to-edge instead of `Mark.BAR`'s separated bands.
...
different chart type that happens to share a name with this package's pre-existing `ARC`) is the first mark drawn from an *edge list*
...
ARC`'s start-at-12-o'clock, sweep-clockwise ring-sector convention
...
each rim, plus a `Path.quad_curve_to` pulled toward the circle's center for each cross connection) drawn through `DrawTarget.
fill_path_aa`. See
chord.mojo's docstrings for the per-node running-angle bookkeeping
...
first mark with only *one* axis drawn at all: `encode_single_axis`'s `x` (plus the usual optional `color`/`color_categories`/`size`
...
reuses `Mark.POINT`'s `_draw_point_layer` completely unchanged.
...
axis frame at all -- it's `Mark.POINT`'s continuous x/y encoding
...
drawn under each point first. A static stand-in for ECharts' real
...
"highest to lowest" ordering, not the caller's row order every
other categorical mark here keeps), each row's bottom width equal
to the *next* row's top width for a continuous taper. See
`_render_funnel`'s docstring for the row-height/palette-by-
...
STREAMGRAPH (the last of Phase 2) reuses `Mark.STACKED_BAR`'s running-total stacking over `encode_grouped_bar`'s data, but each
category's stack starts from `-total_i / 2` instead of a shared
zero (`_symmetric_zero_baseline_y_extent`, streamgraph.mojo -- the
same "forced symmetric" reasoning `POPULATION_PYRAMID`'s x-domain
...
`DrawTarget.fill_path_aa`) instead of `STACKED_BAR`'s discrete
rects. Reuses `_draw_categorical_axis_frame` unchanged.

BEESWARM (the first of "Phase 3", the distribution-shape family --
see the wiki) has its new `encode_distribution` (categories, one
...
its category's band to avoid overlap (`_beeswarm_offsets`,
beeswarm.mojo -- a deterministic row-clustering swarm, not a full
physics-style one; see that function's docstring for why).
...
VIOLIN reuses `BEESWARM`'s `encode_distribution` unchanged, drawing
...
`_kde_bandwidth`'s docstring, violin.mojo) for the KDE's bandwidth by default, or `mark_violin()`'s optional `bandwidth`
...
to its sample size), sampled at `_KDE_SAMPLES` points across each
category's `[min, max]`, each violin's peak density
independently scaled to `theme.violin_width_fraction` of its band width
by default (ggplot2's `scale = "width"`), or `mark_violin()`'s `scale_by_count=True` for ggplot2's `scale = "area"` instead
...
RIDGELINE (the last of Phase 3) reuses `VIOLIN`'s `_kde_bandwidth`/
`_kde_density`/`_KDE_SAMPLES` completely unchanged (including its optional `bandwidth`/`scale_by_count` overrides, `mark_ridgeline()`'s parameters of the same names), but on `GANTT`'s horizontal categorical frame instead of the vertical one -- called
with `padding=0.0` (that function's default is 0.2, right for
`GANTT`'s separated floating bars but not this: rows need to sit
...
frame`'s docstring for the bug this fixed): each category's curve rises upward from its row's bottom edge (`theme.ridgeline_overlap`
times the row's height, deliberately more than one row tall, so a
tall peak overlaps into the row above -- the defining ridgeline look),
drawn top to bottom in `x_categories`' given order so a lower
row's curve sits on top of an upper row's wherever they overlap.
See ridgeline.mojo's docstring for the full reasoning.

NIGHTINGALE ("Phase 4" of the broader chart-type survey, the polar-
axis family -- see the wiki) reuses `ARC`'s `encode_categorical`
...
instead of `ARC`'s value-proportional one -- a wedge's magnitude is
encoded by its radius instead, scaled against the data's largest
value (`Plot.mark_nightingale(area=...)`'s `False`/`"radius"` mode
scales linearly; `True`/`"area"` mode scales by `sqrt(value / max)`
instead, so a wedge's *area*, not just its radius, is proportional
to its value -- ECharts' second `rose_type`). See nightingale.
mojo's `_render_nightingale` docstring for the full reasoning.
...
distinction `HEATMAP`'s docstring already draws against `BAR`,
applied here against `NIGHTINGALE`'s touching sectors) so bars
...
`rose_type="area"` equivalent; ECharts' `polarbar` has no such
mode. See polar_bar.mojo's `_render_polar_bar` docstring.

POLAR is the odd one out among the polar-axis family: not a
categorical mark at all, but the polar equivalent of `LINE` -- its new `encode_polar()` maps a continuous `angle` (radians, used exactly
as given, never wrapped `mod 2*pi` -- so a caller plotting angle
values past a full turn gets a real spiral, ECharts.jl's "spiral"
example) and a continuous `radius` (linearly scaled from `[0,
max(radius)]`, always zero-anchored at the chart's center) onto
...
version gets its encode method" precedent `encode_grouped_bar()`
set alongside `encode_categorical()`, `polar()`/`polar_series()` then
mirroring `pie()`/`donut()`'s "one mark, two quickplot entry
points" split. See polar.mojo's `_render_polar` docstring for the
full reasoning, including the remaining deliberate v1 scope cut (no
axis tick labels).

RADAR is the polar-axis family's categorical mark: one spoke per
named indicator (`encode_radar()`'s `indicators`, each with its `max_values[i]` -- unlike `POLAR`'s single shared radius domain,
...
Reuses `POLAR`'s `_polar_point`, but its grid (`_draw_radar_
grid`, radar.mojo) draws straight-edged polygon "web" rings, not
`POLAR`'s circles -- a radar's axes are discrete, so there's no
meaningful position between two spokes for a circle to pass through.
See radar.mojo's `_render_radar` docstring for the full reasoning.
...
dial -- green/blue/red at ECharts' default 20%/80%/100%
breakpoints, or a caller's `breakpoints`/`band_colors` (an
...
`encode()`'s optional `color`/`size` channels already use --
originally fixed constants, made configurable once a real reason
showed up, see gauge.mojo's docstring). Reuses `POLAR`/`RADAR`'s shared `_polar_point` for both the needle tip and the band
boundaries, `ARC`'s `fill_ring_sector_aa` primitive for the bands
themselves. No axis frame, no legend -- a single value has no
categories to key one by. See gauge.mojo's `_render_gauge`
docstring for the full reasoning.

PARALLEL ("Phase 5" of the broader chart-type survey, its new
N-axis core) is a cartesian, not polar, multi-axis mark: `encode_
parallel()`'s `dims` (one vertical axis per name, evenly spaced
left to right, each independently scaled to its column's `[min, max]` -- unlike `RADAR`'s caller-supplied per-indicator max)
and `row_names`/`data` (one polyline per named row, one value per
dimension -- the same "outer list indexes named things" shape `RADAR`'s `series_values` already established). No axis tick labels beyond
each dimension's name -- the same deliberate v1 simplification
`POLAR`'s grid already documents. See parallel.mojo's `_render_parallel` docstring for the full reasoning.

SPAN_CHART (investigated during Phase 2, built later once the polar
family and Phase 5 were done) is `GANTT`'s mirror image: the exact
same `encode_gantt()` data (category + start + end) completely
unchanged, but drawn as a floating *vertical* bar per category on the
normal `_draw_categorical_axis_frame` instead of `GANTT`'s horizontal one -- ECharts.jl's `spanchart`, useful for a range
that isn't anchored to zero (confidence intervals, daily temperature
highs/lows). See span_chart.mojo's `_render_span_chart` docstring
for the full reasoning.

CALENDAR_HEATMAP ("Phase 6" of the broader chart-type survey, the
grid/matrix family that extends `HEATMAP`'s grid-cell idea) lays
`encode_calendar()`'s `dates`/`values` out in a GitHub-
...
package deliberately still doesn't have (see `GANTT`'s docstring).
See calendar_heatmap.mojo's `_render_calendar_heatmap` docstring
for the full reasoning.

CORRPLOT (second of Phase 6) reuses `HEATMAP`'s `_draw_grid_axis_
frame` directly -- unlike `HEATMAP`'s two independent category
domains, `encode_corrplot()`'s `variables` list labels *both*
axes (a correlation matrix is always square). One bubble per
surviving cell, radius scaling with `abs(matrix[row][col])`, color
through `HEATMAP`'s continuous gradient but spanning the fixed
`[-1.0, 1.0]` correlation domain, not a data-derived one. `Plot.
mark_corrplot(layout=..., diag=..., labels=...)` controls which
cells draw and whether each one's value is labeled -- the same
lower/upper-triangle convention ECharts.jl's `corrplot()` uses.
See corrplot.mojo's `_render_corrplot` docstring for the full
reasoning.

PUNCHCARD (third of Phase 6) reuses `HEATMAP`'s `_draw_grid_axis_
frame` and `_categorical_indices` domain derivation unchanged -- a
"scatter plot on a categorical grid" (ECharts.jl's description),
one bubble per row instead of `HEATMAP`'s filled cell, magnitude
read from bubble *size* (`sizes[i] / scale`, a plain pixel-space
divisor, `Plot.mark_punchcard(scale=10.0)`'s default) rather than
color -- unlike `CORRPLOT`'s cell-normalized bubble sizing, a
punchcard's bubbles are meant to visually overflow a small cell when
the underlying count is large. A repeated `(x, y)` pair draws its independent bubble rather than merging, and there's no legend (no
categorical color channel to key one by). See punchcard.mojo's `_render_punchcard` docstring for the full reasoning.

MARIMEKKO (fourth and last of Phase 6) generalizes `STACKED_BAR`'s "one stacked column per category" idea to two dimensions at once:
`encode_marimekko()`'s `categories` (columns) and `subcategories`
(stacked segments) both carry real proportions -- column *widths* are
each category's share of the grand total, and each column's segment *heights* are that column's subcategory composition (a
0-100% stack), not `STACKED_BAR`'s equal-width columns with a
...
`STACKED_BAR`'s `_axis_pixel`-per-running-total approach already
uses, so adjacent columns/segments never show a hairline gap. See
marimekko.mojo's `_render_marimekko` docstring for the full
reasoning.
...
`d3.stratify()`-style flattened tree (`encode_hierarchy()`'s `ids`/`parent_ids`/`values`), the same "the data already says what's
needed" precedent `encode_chord()`'s edge list already set for
graphs, generalized to trees. Drawn as concentric `fill_ring_sector_
aa` rings (`ARC`'s primitive, reused directly) -- one ring per
depth level, each node's angular span proportional to its share of its *parent's* own subtree total (not the grand total the
way `NIGHTINGALE`'s wedges are), so a sunburst is a multi-level
generalization of `NIGHTINGALE`'s single-level rose. Every
descendant of one of the root's direct children shares that
child's palette color, so one glance shows which top-level branch
a deeply nested ring segment belongs to. See sunburst.mojo's `_render_sunburst` docstring for the full reasoning.
...
picks each node's row, `_assign_leaf_positions` (tree.mojo -- a
deliberately simplified layout, *not* a real Reingold-Tilford
algorithm, see its docstring) picks each node's column (every
leaf gets the next sequential slot, left to right; an internal node
sits at the plain average of its children's slots). The same
...
two separate passes rather than one recursive one. See tree.mojo's `_render_tree` docstring for the full reasoning.
...
child's share of a split proportional to its `subtree_value`
share of its parent's total) -- a deliberately simplified layout, not
a real squarified algorithm (see that function's docstring, the
same "simpler, still genuinely correct" tolerance `TREE`'s layout
...
mojo's `_render_treemap` docstring for the full reasoning.

ARC_DIAGRAM (first of Phase 7's edge-list sub-family) reuses `CHORD`'s `encode_chord()` edge-list data completely unchanged (the same
"identical data, purely a rendering difference" precedent `STACKED_
BAR`'s reuse of `GROUPED_BAR` already established) -- not this
package's pre-existing `ARC` (pie/donut wedges), a genuinely
different chart type that happens to share a name, the same naming
collision `CHORD`'s docstring already notes. Every distinct node
sits on one straight line instead of `CHORD`'s circle, edges drawn
...
edge and node marker color both follow the edge's `from` node's
palette color, the same convention `CHORD`'s ribbons already use.
Every node is labeled directly beneath its marker -- no legend,
unlike `CHORD` (which needs one since its ring sectors are often
too thin to label directly). See arc_diagram.mojo's `_render_arc_
diagram` docstring for the full reasoning.

GRAPH (second of Phase 7's edge-list sub-family) reuses `CHORD`'s edge-list data too, a
third genuinely different network layout: nodes evenly spaced around
a circle (`ARC`'s start-at-12-o'clock convention, reused for
position only), edges drawn as straight lines cutting across the
interior instead of `CHORD`'s ring-sectors-plus-ribbons or `ARC_
DIAGRAM`'s nodes-on-a-line-plus-arcs. A deliberately simple,
deterministic circular layout, not a real force-directed simulation --
the same "not a full physics simulation" scope choice `BEESWARM`'s docstring already makes for a completely different layout
...
of center they fall on, the same rule `RADAR`'s axis labels
already use. See graph.mojo's `_render_graph` docstring for the
full reasoning.

SANKEY (third and last of Phase 7's edge-list sub-family, and the
last mark of Phase 7 overall) reuses `CHORD`'s edge-list data one
more time, laid out left-to-right by column -- a node's column is
the length of the *longest* path reaching it from any source,
computed via one Kahn's-algorithm topological pass (which also
detects a cycle and raises, since a real Sankey's flow data must
...
("ribbon" segments) between a slice of its `from` node's right
edge and its `to` node's left edge -- straight edges, not a
smooth curve, the same "straight, not curved" simplification `CHORD`'s straight-rim ribbons already are. A "skip" edge (spanning more
...
column's vertical space exactly like a real node, so real nodes
...
closed once a real reason showed up; every segment in one flow's chain still colors by that flow's original source, not whichever
...
out). See sankey.mojo's `_render_sankey` docstring for the full
reasoning.

RADIALBAR (the `polarbar` gap item scoped out of Phase 4, closed once
Phase 7 wrapped) reuses `POLAR_BAR`'s `encode_categorical` data
...
a light-gray track, instead of `POLAR_BAR`'s bars radiating
outward from a shared center -- the "multi-ring progress indicator"
shape (Apple Watch's activity rings, GitHub's contribution-
ring widgets), not a circular column chart. The first category's ring draws outermost, each later one nesting further in -- display
prominence, not `SUNBURST`'s hierarchy-depth ordering (there's no
hierarchy here). See radialbar.mojo's `_render_radialbar`
docstring for the full reasoning.
"""
