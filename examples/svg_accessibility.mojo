...
element, plus <title> (and, when given, <desc>) as its leading
child elements -- exactly what the SVG accessibility spec, and every
screen reader that supports SVG at all, looks for.

Only helps when the SVG's accessible tree actually gets walked --
inline <svg> markup, a standalone .svg opened directly, or an
<object>/<iframe> embed all expose it. A plain <img src="chart.svg">
(how this project's docs site embeds every other example) does
not -- see accessible_svg_string()'s docstring for the full story,
which is also why this one example gets its dedicated page here
rather than being folded into every other chart's file.
