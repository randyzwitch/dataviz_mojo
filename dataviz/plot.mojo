...
Seams of this file that are self-contained enough to stand alone are
being split out the same way (#222), starting with the `annotate_*()`
overlays in `annotations.mojo`. They are imported back here, so where
a symbol lives is not something a caller has to know.
