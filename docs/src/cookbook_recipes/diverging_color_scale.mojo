...
def main() raises:
    ...
    var temp_anomaly: List[Float64] = [-1.8, -0.6, -1.2, 0.4, 1.5, -0.2, 0.9, 2.1]

    var plot = (
        Plot()
        .mark_point()
        .encode(x=x, y=y, color=temp_anomaly)
        .labels(title="Readings by Temperature Anomaly", subtitle="Deviation from the 30-year average, °C")
        .theme(
            Theme(
