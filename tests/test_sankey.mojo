...
chord()'s own shared validation (raster + SVG) -- see sankey.mojo's
own docstrings for the rules verified here.
"""
...
def test_render_sankey_matches_hand_derived_nodes_and_ribbon() raises:
    # 2 nodes (A, B), one edge A->B (value 10 -- both node's own
    # entire inflow/outflow, so no imbalance gap). Canvas 400x300,
    ...
    # node-width so the last column's own node doesn't clip past
    # plot_x1). Node A's own rect: (60,20,12,230); node B's own rect:
    # (368,20,12,230) -- both fill the plot's own full height (the
    # only node in their own column). The ribbon fills the gap
    # between them, (72,20)-(72,250)-(368,250)-(368,20), colored by
    # its own "from" node (A). Every number confirmed against a real
    # render_svg() run first (see this file's own SVG test).
    ...
    _assert_color(c, 66, 135, palette[0], "node A's own rect")
    _assert_color(c, 220, 135, palette[0], "the ribbon, well inside its own bounds")
    _assert_color(c, 374, 135, palette[1], "node B's own rect")
...
def test_render_sankey_svg_matches_confirmed_geometry() raises:
    ...
    assert_true(
        ...
        "the ribbon, A's own column edge to B's own column edge",
    )
...
def test_render_sankey_skip_edge_routes_through_a_pass_through_node() raises:
    ...
    # pass-through node splits its own column exactly in half. Canvas
    ...
    # (B=1, then D's own pass-through node, appended after every real
    ...
    #
    # Ribbons: A->B fills column0-1's own top half (blue, A's own
    # color); B->C fills column1-2's own top half (orange); D's own
    # skip edge becomes two chained segments, D->pass-through
    # (column0-1's own bottom half) and pass-through->C (column1-2's
    # own bottom half), *both* colored green (D's own color, the
    # flow's original source) even though the second segment's own
    # immediate `from` is the invisible pass-through node, not D
    ...
    var palette = default_categorical_palette()
    _assert_color(c, 66, 77, palette[0], "node A's own rect (column 0, top half)")
    _assert_color(c, 66, 192, palette[2], "node D's own rect (column 0, bottom half)")
    _assert_color(c, 220, 77, palette[1], "node B's own rect (column 1, top half)")
    _assert_color(c, 374, 135, palette[3], "node C's own rect (column 2, full height)")
    ...
    _assert_color(
        c, 143, 192, palette[2], "D's own skip edge, first segment (D -> pass-through), still D's own color"
    )
    _assert_color(
        c, 297, 192, palette[2], "D's own skip edge, second segment (pass-through -> C), still D's own color"
    )
    # The one point that actually distinguishes this from the pre-fix
    # behavior: (220, 192) sits inside column 1's own node-width strip
    # (x 214-226), in its own bottom half -- the pass-through node's
    ...
    # width gap itself) -- background. Before this fix, D's own skip
    # edge drew as one straight ribbon with no pass-through node
    # reserving column 1 space at all, so B's own bar -- the column's
    # only "real" member -- claimed the *entire* column height instead
    # of just its own top half, painting orange here (confirmed by
    # deliberately reverting to that behavior and re-probing before
    # writing this assertion).
    _assert_color(c, 220, 192, BG, "the pass-through node's own reserved slot -- background, not node B's bar")
