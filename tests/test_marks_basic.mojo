...
def test_encode_histogram_bins_match_hand_derived_counts() raises:
    ...
    assert_equal(len(plot.mark.categorical.x), 5)
    assert_equal(plot.mark.categorical.x[0], "1.0-2.6")
    assert_equal(plot.mark.categorical.x[1], "2.6-4.2")
    assert_equal(plot.mark.categorical.x[2], "4.2-5.8")
    assert_equal(plot.mark.categorical.x[3], "5.8-7.4")
    assert_equal(plot.mark.categorical.x[4], "7.4-9.0")
    assert_equal(plot.mark.continuous.y[0], 3.0)
    assert_equal(plot.mark.continuous.y[1], 3.0)
    assert_equal(plot.mark.continuous.y[2], 2.0)
    assert_equal(plot.mark.continuous.y[3], 0.0)
    assert_equal(plot.mark.continuous.y[4], 2.0)
...
def test_encode_histogram_rule_matches_hand_derived_bins() raises:
    ...
    assert_equal(len(plot.mark.categorical.x), 4)
    assert_equal(plot.mark.categorical.x[0], "0.0-3.0")
    assert_equal(plot.mark.categorical.x[3], "9.0-12.0")
    for i in range(4):
        assert_equal(plot.mark.continuous.y[i], 4.0)
...
def test_encode_histogram_rule_agrees_with_the_numeric_path() raises:
    ...
    assert_equal(len(plot.mark.continuous.y), len(numeric.values))
    for i in range(len(numeric.values)):
        assert_equal(plot.mark.continuous.y[i], numeric.values[i])
...
def test_encode_histogram_centers_bins_on_a_constant_sample() raises:
    ...
    var plot = Plot().mark_bar().encode_binned_categories(data, bins=5)
    assert_equal(len(plot.mark.categorical.x), 5)
    assert_equal(plot.mark.categorical.x[0], "4.5-4.7")
    assert_equal(plot.mark.categorical.x[2], "4.9-5.1")
    assert_equal(plot.mark.categorical.x[4], "5.3-5.5")
    assert_equal(plot.mark.continuous.y[0], 0.0)
    assert_equal(plot.mark.continuous.y[1], 0.0)
    assert_equal(plot.mark.continuous.y[2], 3.0)
    assert_equal(plot.mark.continuous.y[3], 0.0)
    assert_equal(plot.mark.continuous.y[4], 0.0)
