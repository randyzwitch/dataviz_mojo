...
def test_render_polar_matches_hand_derived_line_and_markers() raises:
    ...
    var c = render(polar(angle, radius, width=400, height=300))
...
def test_render_polar_draws_a_grid_even_with_no_data_on_it() raises:
    ...
    # outermost ring (220, 32), which must differ from a plain white
    # background (see _render_polar's docstring: no tick labels, but
    # the rings/spokes themselves always draw). A loose inequality
    # rather than an exact gridline-color match since AA blending on a
    # 1px stroke can still land a fraction short of the full color.
    var angle: List[Float64] = [0.0]
    var radius: List[Float64] = [0.0]
    var c = render(polar(angle, radius, width=400, height=300))
    var p = c.get_pixel(220, 32)
    assert_true(
...
def test_render_polar_raises_on_mismatched_length() raises:
    ...
    with assert_raises():
        _ = render(polar(angle, radius, width=200, height=150))
...
def test_render_polar_empty_data_only_fills_background() raises:
    ...
    var c = render(polar(angle, radius, width=100, height=80))
...
def test_render_polar_series_matches_hand_derived_line_and_markers() raises:
    ...
    var c = render(polar_series(angle, names, vals, width=400, height=300))
...
def test_render_polar_series_raises_on_mismatched_names_and_values_length() raises:
    ...
    with assert_raises():
        _ = render(polar_series(angle, names, vals, width=200, height=150))
...
def test_render_polar_series_empty_angle_only_fills_background() raises:
    ...
    var c = render(polar_series(angle, names, vals, width=100, height=80))
    _assert_color(c, 50, 40, BG, "no data: nothing drawn but the background")
