...
fixed row (raster + SVG) -- see single_axis.mojo's own docstrings for
...
def test_render_single_axis_matches_hand_derived_points() raises:
    ...
    var c = single_axis(x, theme=t, width=400, height=300)
    ...
    _assert_color(c, 75, 100, BG, "same column as the first point, but off its own row -- background")
...
def test_render_single_axis_color_encoding_reuses_point_channels() raises:
    ...
    # [0, 10] domain -- confirms Mark.POINT's own _draw_point_layer
    ...
    var c = single_axis(x, color=color, theme=t, width=400, height=300)
    _assert_color(c, 75, 135, t.color_scale_low, "x=0, color=0.0 -- the color domain's own min")
    _assert_color(c, 365, 135, t.color_scale_high, "x=10, color=10.0 -- the color domain's own max")
...
def test_render_single_axis_raises_on_mismatched_channel_length() raises:
    ...
    with assert_raises():
        _ = single_axis(x, color=color, width=200, height=150)


def test_render_single_axis_empty_data_only_fills_background() raises:
    var x = List[Float64]()
    var c = single_axis(x, width=200, height=150)
