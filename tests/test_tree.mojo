...
positioning, per-branch edge/marker color, encode_hierarchy()'s own
shared validation (raster + SVG) -- see tree.mojo's own docstrings for
...
def test_render_tree_matches_hand_derived_positions() raises:
    ...
    # exact canvas size). 2 leaves -> A's own slot 0 maps to plot_x0
    # (60), B's own slot 1 maps to plot_x1 (380); root's own slot is
    ...
    _assert_color(c, 220, 20, t.text_color, "root's own marker -- no branch, stays text_color")
    _assert_color(c, 60, 250, palette[0], "A's own marker -- root's own first child, palette[0]")
    _assert_color(c, 380, 250, palette[1], "B's own marker -- root's own second child, palette[1]")
    # A point along the root->A edge, well clear of either marker's
    # own radius: the edge from (220,20) to (60,250), at its own 25%
...
def test_render_tree_svg_matches_confirmed_geometry() raises:
    ...
    assert_true('<circle cx="220" cy="20" r="4" fill="#282828"/>' in s, "root's own marker")
    assert_true('<circle cx="60" cy="250" r="4" fill="#1f77b4"/>' in s, "A's own marker")
    assert_true('<circle cx="380" cy="250" r="4" fill="#ff7f0e"/>' in s, "B's own marker")
