...
auto-scaling, polyline geometry, encode_parallel()'s own length
validation.
"""
...
def test_render_parallel_matches_hand_derived_polylines() raises:
    ...
    # set each column's own domain to a clean [0, 10] without
    # themselves landing at a boundary pixel this test samples (a
    # point exactly at the plot's own edge is prone to AA/stroke-cap
    ...
    # mojo's own tests already derive for this exact canvas size. Two
    # axes (n=2) pin to the plot's own left/right edges: A at x=60, B
    # at x=380.
    #
    # r1 = [3, 7]: A's own frac 3/10=0.3 -> y = 250 - 0.3*230 = 181.
    # B's own frac 7/10=0.7 -> y = 250 - 0.7*230 = 89. r2 = [7, 3]:
    # the mirror image, A -> y=89, B -> y=181. Both endpoints (the
    # polyline's own first vertex, x=60) and an interior point 25% of
    ...
    _assert_color(c, 60, 181, palette[0], "r1's own first vertex, axis A (frac 0.3)")
    _assert_color(c, 140, 158, palette[0], "r1's own polyline, 25% of the way to axis B")
    _assert_color(c, 60, 89, palette[1], "r2's own first vertex, axis A (frac 0.7)")
    _assert_color(c, 140, 112, palette[1], "r2's own polyline, 25% of the way to axis B")
