...
to be one big test_plot.mojo -- kept in its own module (not a *.mojo
file with its own main()) so every split-out test file can import
...
importable package, not one -- see dataviz_mojo/__init__.mojo's own
docstring/pixi.toml's own package-declaration comment for why tests/
examples can never become a package themselves, since every file in
here has its own main() and Mojo refuses to `mojo package`/`mojo
...
import ...` -- see pixi.toml's own test task.
"""
